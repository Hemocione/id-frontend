...
const LoginSection = () => {
    ...
    const handleSubmit = (e) => {
        ...
        window.grecaptcha.ready(() => {
            window.grecaptcha.execute(process.env.NEXT_PUBLIC_SITE_KEY, { action: 'submit' }).then(async captchaToken => {
    ...
    const apiRecover = async (captchaToken) => {
        try {
            ...
            setLoading(false)
            setErrorText((error.response && error.response.data.message) || "Ocorreu um erro inesperado. Por favor, tente novamente.")
        }
    }
            ...
            <form onSubmit={handleSubmit}>
                <div className={styles.recoverContent}>
                    <div className={styles.title}>
                        <Image src='/vertical-cor-fb.svg' width={150} height={150} alt="Hemocione Logo" />
                    </div>
                    <p className={styles.errorText}>{errorText}</p>
                    {sent
                        ? <div>
                            <h2 className={styles.finalMessage}>
                                Verifique sua caixa de entrada e spam, enviamos um email de recuperação de conta
                            </h2>
                            <SimpleButton onClick={handleReturn} passStyle={{ width: '100%' }}>
